...
//Applying binary search algorithm.
    ...
    //Applying sorting algorithm according to room no.
...
//Applying sorting algorithm lexicographically

    bool comparename(patient w1,patient w2)
    {
        if(w1.name<w2.name)
        {
            return 1;
...
int main()
{
    ...
    while(ok==true)
    {
            ...
            {
                if(a[i]!=0&&a[i]<14)
                {      //checking if the quarantine period is over
                    a[i]++;
                }
                else if(a[i]>=14)
                {
        ...
        if(ans1=="yes")
        {                                                //Entering the details of the patients
            ...
            for(int i=0;i<n1;i++)
            {
                ...
                {
                    p.alloted_room_no=q1.front();       //the first element of the queue will get alloted to the patient coming in
                    cout<<"Alloted room no is: ";
                    cout<<p.alloted_room_no<<endl;
                    a[p.alloted_room_no]++;
                    q1.pop();                   //the alloted room will get popped out from the queue q1
                    q2.push(p);                 //the room will get alloted to the patient
                    v.push_back(p);
                    sor.push_back(p);
        ...
        if(ans6=="yes")
        {
            ...
            {
                if(a[i]==0)
                {                      //displaying the empty rooms since array a is used to know the no of days a room
                                                      //is under quarantine
        ...
        if(ans2=="yes")
        {
            sort(v.begin(),v.end(),comparename); //calling the sort function
        ...
        if(ans4=="yes")
        {
            ...
            {
                cout<<"The patient's name is "<<'\n'<<sor[i].name<<'\n'<<"The alloted room no is"<<'\n'
                <<sor[i].alloted_room_no<<endl;
        ...
        cin>>ans5;
        if(ans5=="yes")
        {            //asking if user wants to continue for the next day
            ok=false;
        }
        else
            ok=true;
